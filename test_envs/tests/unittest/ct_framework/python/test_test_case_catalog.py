...
class TestCaseCatalogTests(unittest.TestCase):
    ...
    def test_tool_catalogs_register_implemented_tools(self) -> None:
        interface_value = json.loads(
            Path("test_envs/configs/pytest/test_interfaces_catalog.json").read_text(encoding="utf-8")
        )
        equipment_value = json.loads(
            Path("test_envs/configs/pytest/test_equipments_catalog.json").read_text(encoding="utf-8")
        )
