...
class TestCaseCatalogTests(unittest.TestCase):
    def test_ct_catalog_registers_uart_usb_and_network(self) -> None:
        value = json.loads(
            Path("test_envs/tests/pytest/test_cases/catalog.json").read_text(encoding="utf-8")
        )
    ...
    def test_tool_catalogs_register_implemented_tools(self) -> None:
        interface_value = json.loads(
            Path("test_envs/tests/pytest/test_interfaces/catalog.json").read_text(encoding="utf-8")
        )
        equipment_value = json.loads(
            Path("test_envs/tests/pytest/test_equipments/catalog.json").read_text(encoding="utf-8")
        )
